    ...
    <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <select id="religion" name="religion" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">
                        ...
                        @if(isset($religions))
                            @foreach($religions as $religion)
                                <option value="{{ $religion->name }}" {{ old('religion') == $religion->name ? 'selected' : '' }}>
                                    {{ $religion->name }}
                                </option>
                            @endforeach
                        @endif
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <select id="rank" name="rank" 
                        ...
                        @if(isset($ranks))
                            @foreach($ranks as $rank)
                                <option value="{{ $rank->name }}" {{ old('rank') == $rank->name ? 'selected' : '' }}>
                                    {{ $rank->code }} - {{ $rank->name }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <select id="grade" name="grade" 
                        ...
                        @if(isset($grades))
                            @foreach($grades as $grade)
                                <option value="{{ $grade->code }}" {{ old('grade') == $grade->code ? 'selected' : '' }}>
                ...
                <div>
                    <label for="village_id" class="block text-sm font-medium text-gray-700 mb-2">Penempatan Desa *</label>
                        ...
                        @if(isset($educations))
                            @foreach($educations as $education)
                                <option value="{{ $education->id }}" {{ old('education_id') == $education->id ? 'selected' : '' }}>
                                    {{ $education->level }} - {{ $education->name }}
                                </option>
                            @endforeach
                        @endif
                    </select>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mt-4">
                ...
                <input type="text" id="tanda_tangan_sk" name="tanda_tangan_sk" value="{{ old('tanda_tangan_sk') }}" 
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
